...
#define CROSSPARK_SIGNALID_START 15	
#define CROSSPARK_SIGNALID_FINISH 17
...
#define CP_DISTANCE_ADD1 "initCP::DISTANCE_ADD1"
#define CP_DISTANCE_ADD2 "initCP::DISTANCE_ADD2"
#define CP_DISTANCE_ADD3 "initCP::DISTANCE_ADD3"
#define CP_DISTANCE_ADD4 "initCP::DISTANCE_ADD4"
#define CP_DISTANCE_ADD5 "initCP::DISTANCE_ADD5"
#define CP_DISTANCE_ADD6 "initCP::DISTANCE_ADD6"
#define CP_DISTANCE_ADD7 "initCP::DISTANCE_ADD7"
#define CP_PARK_ACCEL "initCP::PARK_ACCEL"
#define CP_PARK_ACCEL_REV "initCP::PARK_ACCEL_REV"
#define CP_STG3STR "initCP::STG3STR"
#define CP_ENABLE_IMSHOW  "initCP::Enableimshow"
#define MD_REQUIRED_MARKER_SIZE "initCP::Required marker size"
#define PP_ENBF "initPP::ENBF"

ADTF_FILTER_PLUGIN("cross Park Initialize",OID_ADTF_DEMO_START_CROSS_PARK, ccrossParkInit)



ccrossParkInit::ccrossParkInit(const tChar* __info):cFilter(__info)
{   
    ...
    SetPropertyBool(CP_ENABLE_IMSHOW,imshowflag);
    SetPropertyStr(CP_ENABLE_IMSHOW NSSUBPROP_DESCRIPTION, "If true imshow will be enabled");

    SetPropertyFloat(CP_DISTANCE_ADD1, 0.26);
    SetPropertyBool(CP_DISTANCE_ADD1 NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_DISTANCE_ADD1 NSSUBPROP_DESCRIPTION, "stage1 add distance");

    SetPropertyFloat(CP_DISTANCE_ADD2, 0.4);
    SetPropertyBool(CP_DISTANCE_ADD2 NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_DISTANCE_ADD2 NSSUBPROP_DESCRIPTION, "stage2 add distance");

    SetPropertyFloat(CP_DISTANCE_ADD3, 0.55);
    SetPropertyBool(CP_DISTANCE_ADD3 NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_DISTANCE_ADD3 NSSUBPROP_DESCRIPTION, "stage3 add distance");

    SetPropertyFloat(CP_DISTANCE_ADD4, 0.65);
    SetPropertyBool(CP_DISTANCE_ADD4 NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_DISTANCE_ADD4 NSSUBPROP_DESCRIPTION, "stage4 add distance");

    SetPropertyFloat(CP_DISTANCE_ADD5, 0.65);
    SetPropertyBool(CP_DISTANCE_ADD5 NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_DISTANCE_ADD5 NSSUBPROP_DESCRIPTION, "stage5 add distance");

    SetPropertyFloat(CP_DISTANCE_ADD6, 0.2);
    SetPropertyBool(CP_DISTANCE_ADD6 NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_DISTANCE_ADD6 NSSUBPROP_DESCRIPTION, "stage6 add distance");

    SetPropertyFloat(CP_PARK_ACCEL, 83);
    SetPropertyBool(CP_PARK_ACCEL NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_PARK_ACCEL NSSUBPROP_DESCRIPTION, "parking accel");


    SetPropertyFloat(CP_PARK_ACCEL_REV, 99);
    SetPropertyBool(CP_PARK_ACCEL_REV NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_PARK_ACCEL_REV NSSUBPROP_DESCRIPTION, "parking accel reverse");

    SetPropertyFloat(CP_STG3STR, 77);
    SetPropertyBool(CP_STG3STR NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(CP_STG3STR NSSUBPROP_DESCRIPTION, "stage 3 steering angle");
...
ccrossParkInit::~ccrossParkInit()
{
}

tResult ccrossParkInit::PropertyChanged(const char* strProperty)
{
    ReadProperties(strProperty);
    RETURN_NOERROR;
}

tResult ccrossParkInit::ReadProperties(const tChar* strPropertyName)
{
    
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_DISTANCE_ADD1))
    {
        distance_add1 = GetPropertyFloat(CP_DISTANCE_ADD1);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_DISTANCE_ADD2))
    {
        distance_add2 = GetPropertyFloat(CP_DISTANCE_ADD2);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_DISTANCE_ADD3))
    {
        distance_add3 = GetPropertyFloat(CP_DISTANCE_ADD3);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_DISTANCE_ADD4))
    {
        distance_add4 = GetPropertyFloat(CP_DISTANCE_ADD4);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_DISTANCE_ADD5))
    {
        distance_add5 = GetPropertyFloat(CP_DISTANCE_ADD5);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_DISTANCE_ADD6))
    {
        distance_add6 = GetPropertyFloat(CP_DISTANCE_ADD6);
    }
    /*if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_ACCEL_TEST_VAL))
    {
        accel_test_val = GetPropertyFloat(CP_ACCEL_TEST_VAL);
    }*/
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_PARK_ACCEL))
    {
        parkaccel = GetPropertyFloat(CP_PARK_ACCEL);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_PARK_ACCEL_REV))
    {
        parkaccelrev = GetPropertyFloat(CP_PARK_ACCEL_REV);
    }
    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_STG3STR))
    {
        stg3str = GetPropertyFloat(CP_STG3STR);
    }

    if (NULL == strPropertyName || cString::IsEqual(strPropertyName, CP_ENABLE_IMSHOW ))
    {
    	imshowflag = GetPropertyBool(CP_ENABLE_IMSHOW);
    }
...
tResult ccrossParkInit::Init(tInitStage eStage, __exception)
{
...
tResult ccrossParkInit::Start(__exception)
{
	return cFilter::Start(__exception_ptr);
}

tResult ccrossParkInit::Stop(__exception)
{	
	imshowflag = tFalse;
	return cFilter::Stop(__exception_ptr);
}

tResult ccrossParkInit::Shutdown(tInitStage eStage, __exception)
{
...
tResult ccrossParkInit::OnPinEvent(IPin* pSource,
                                            tInt nEventCode,
                                            tInt nParam1,
                                            tInt nParam2,
    ...
    switch (nEventCode)
            ...
            case IPinEventSink::PE_MediaSampleReceived:
				...
				else if (pSource == &m_osignDetect && m_pDescriptionRoadSign != NULL)
				{
					...
					if(( MARKER_ID_PARKINGAREA == i16ID ) && (f32Area > f32RequiredArea) && (JuryID == MANEUVER_CROSS_PARKING)) //REQUIRED_AREA)
					{
				...
				{
					if(JuryID != MANEUVER_CROSS_PARKING){
					}
					else if(maneuverstart_flag && maneuverfinish_flag)
					{
						LOG_INFO(cString::Format("finish set"));
						SignalID = CROSSPARK_SIGNALID_FINISH;
						maneuverstart_flag = tFalse;
					...
					else if(markercheckPassFlag && !maneuverfinish_flag)
					{
						accelout = parkaccel;	
						Steerout = Steerin;
						SignalID = CROSSPARK_SIGNALID_START;
...
tVoid ccrossParkInit::thinningIteration(cv::Mat& img, tInt iter)
{
 ...
 */
tVoid ccrossParkInit::thinning(const cv::Mat& src, cv::Mat& dst)
{
    dst = src.clone();
...
tResult ccrossParkInit::ProcessVideo(adtf::IMediaSample* pISample)
{
    ...
    if (IS_OK(pISample->Lock(&l_pSrcBuffer)))
    {
    		...
    		if(!direct_flag)
		{
			...
			img = TheInputImage ;
			img1 = img;
			//img = img(Rect(430,290,190,30));
			
			img = img(Rect(460,230,180,30));
			//img = img(Rect(430,290,190,30));
			...
			//img1 = img(Rect(103,8,50,20));	//uncomment to save template  
			//imwrite("/home/aadc/AADC/src/aadcUser/src/AADC_Startparallelpark/temp_last.jpg",img1);   //to save template enable
			...
			if(!frame_noise)
			{
				...
				//Rect ROI = cv::Rect( match.x, match.y, mytemplate.cols, mytemplate.rows );
		    		if((abs(match.x - match_old.x) > 20) || (abs(match.y - match_old.y) > 20))
				{
					if(total_count == 0)
					{
					...
					if(!scan)
					{
						...
						{
							if((Distnce-distance_begin) > 0.2)
							{
								total_count = total_count +1;
								distance_begin = Distnce;
							}
							else if((Distnce-distance_begin) > 0.05  && (Distnce-distance_begin) < .2 )
							{
								distance_desired2 = distance_desired2 + 0.1;
								distance_desired3 = distance_desired3 + 0.1;
							}
							foundother = tTrue;
...
tResult ccrossParkInit::DecisionMaking(tUInt32 timeStamp)
{
	switch (currentStage)
		{	
			/*case 1:
			...
			case 2:
			{
				...
				else
				{
					accelout = 90;
					currentStage = 21;
					distance_desired21 = Distnce + distance_add21;
					//maneuverfinish_flag = tTrue;
					LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
				}
				//LOG_INFO(adtf_util::cString::Format(" %f ",distance_desired2));
			...
			case 21:
			{
				...
				else
				{
					accelout = 90;
					currentStage = 22;
					distance_desired22 = Distnce + distance_add22;
					LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
					//maneuverfinish_flag = tTrue;
				}
				break;
			...
			case 22:
			{
				...
				else
				{
					...
					if((!spot) && ( (total_count = count1) || (total_count = (count1 - 1))  ) 	)
					{
						currentStage = 2;
						distance_desired2 = Distnce + distance_add2;
						LOG_INFO(adtf_util::cString::Format("counts total ,spot %d ,%d",total_count, count1));
						spot = tTrue;
						LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
						scan = tFalse;
			...
			/*case 1:
			{
				...
				//LOG_INFO(adtf_util::cString::Format(" %f, distance, %f ",distance_desired2,Distnce));
				//DecisionMaking(timeStamp);
				break;
			}
			case 2:
			{
				...
				else
				{
					...
					currentStage = 21;
					distance_desired21 = Distnce + distance_add21;
					distance_scanstart = Distnce + 0.1;
					distance_scanend = distance_desired21 - 0.1;
					//maneuverfinish_flag = tTrue;
					LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
				}
				//LOG_INFO(adtf_util::cString::Format(" %f, distance, %f ",distance_desired21,Distnce));
				//LOG_INFO(adtf_util::cString::Format(" %f ",distance_desired2));
				//DecisionMaking(timeStamp);
				break;
			}
			...
			case 21:
			{
				...
				if(Distnce < distance_desired21)
				{
					LOG_INFO(adtf_util::cString::Format("USRight %f ", USRight));
					accelout = parkaccel;
					if((Distnce > distance_scanstart) && (Distnce < distance_scanend))
					{
						if(USRight < .55)//sensor distance 
						{
							spot = tFalse;
						}
				...
				//LOG_INFO(adtf_util::cString::Format(" %f, distance, %f ",distance_desired22,Distnce));
				break;
			}
			case 22:
			{
			...
			case 1:
			{
				if(Distnce < distance_desired1)
				{
					accelout = parkaccel; //<-- acceleration for the parking stage
				}
				else
				{
					accelout = 90; //
					currentStage = 2;
					distance_desired2 = Distnce + distance_add2;
					distance_scanend = (distance_desired2 - 0.2);
					//maneuverfinish_flag = tTrue;
				LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
				}
				break;	
...
tResult ccrossParkInit::UpdateInputImageFormat(const tBitmapFormat* pFormat)
{
    if (pFormat != NULL)
    {
...
tResult ccrossParkInit::UpdateOutputImageFormat(const tBitmapFormat* pFormat)
{
    if (pFormat != NULL)
    {
...
/*tResult ccrossParkInit::writeFlags(tUInt32 timeStamp)
{       
...
tResult ccrossParkInit::writeLights(tUInt32 timeStamp)
{       
...
tResult ccrossParkInit::writeOutputs(tUInt32 timeStamp)
{       
