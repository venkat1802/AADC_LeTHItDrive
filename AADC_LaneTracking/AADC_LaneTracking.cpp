...
#define MD_ENABLE_IMSHOW  "LaneDetection::Enableimshow"
    ...
    m_hEmergencyStopTimerResume(NULL)
{
    ...
    SetPropertyFloat(LT_PROP_MAX_ACCELERATION, 79);
    ...
    SetPropertyFloat(LT_PROP_MIN_ACCELERATION, 79);
...
tResult cLaneTracking::Init(tInitStage eStage, __exception )
{
    ...
    else if (eStage == StageNormal)
    {
        ...
        m_f32AccelerateOut = 90.0f;
        m_i16FarLaneWidthCalc = 0;

        m_ui8InitCtrl = 0;
        
        m_f32SteeringAngle = 90.0f;
        m_i16LaneWidth = 390;
        m_nBlindCounter = 0;
    ...
    if(IRunnable::RUN_TIMER == nActivationCode)
    {
        ...
        if(szUserDataSize == sizeof(tUInt8))
        {
            ...
            if(*nUserData == g_ui8StopTimerIdNegative)
            {
                ...
                TransmitAcceleration(90.0f, _clock->GetStreamTime());
...
tResult cLaneTracking::ProcessInput(IMediaSample* pSample, tTimeStamp tsInputTime)
{
    ...
    {
        TransmitAcceleration(90.0f, tsInputTime);
        TransmitSteeringAngle(90.0f, tsInputTime);
...
if(imshowflag) {
        imshow("nearline",matImageCutNear);   
	   waitKey(1);        
}
    ...
    if(imshowflag) {
        imshow("nearline thresh",m_matGreyThreshNear);   
	   waitKey(1);        
}
...
if(imshowflag) {
	   imshow("farline",matImageCutFar);
        ...
        Canny(m_matGreyThreshFar, m_matLineCannyFar, 0, 2, 3, false);// Detect Edges
            if(imshowflag) {
        imshow("farline thresh",m_matGreyThreshFar);   
	   waitKey(1);        
}
        ...
        if(m_bShowDebug)
        {
            if(m_ui8Imagecount > 2)
            {
                ...
                //imshow("GreyScale Image1", m_matGreyThreshNear);
               // imshow("Binary Image", greythreshNear);
//imshow("GreyScale Image2", m_matGreyThreshFar);
...
tResult cLaneTracking::LateralControl(sPoint *psPoints, tUInt8 *pui8PointsCount, tTimeStamp tsInputTime)
{
  ...
  // LOG_INFO(cString::Format("Points Detected : %i", *pui8PointsCount));
...
tResult cLaneTracking::LongitudinalControl(sPoint *psPointsFar, tUInt8 *pui8PointsCountFar, tTimeStamp tsInputTime)
{
    ...
    {
         m_f32AccelerateOut = 90.0;        // If the car can't find lanes anymore --> stop
    }
    else
    {
