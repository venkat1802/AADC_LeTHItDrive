...
//******************************step 1*************************************//
	if(trigger1 == 1)
	{
		...
		start_time = Distnce;  //start distance
		time_diff = 0; 
	...
	if(currentStage == 1)
	{
		...
		end_time = Distnce;	//current distance
		accelout = parkaccel;  
		Steerout = Steerin;
		//time_diff = (float)(end_time - start_time)/1000000; // Converted to Secs //in time
		time_diff = (float)(end_time - start_time); // Converted to Secs  // in distance
		//LOG_INFO(cString::Format("----------Demo : time_diff %f-secs------------",time_diff));	 //time
		LOG_INFO(cString::Format("----------Demo : time_diff %f-m------------",time_diff));	//distance
...
//******************************step 4*************************************//
	...
	if(currentStage == 4)
	{
		...
		Steerout = 120;
		//time_diff = (float)(end_time - start_time)/1000000; // Converted to Secs
		time_diff = (end_time - start_time); // Converted to Secs
		LOG_INFO(cString::Format("----------Demo : time_diff %f-secs------------",time_diff));	
