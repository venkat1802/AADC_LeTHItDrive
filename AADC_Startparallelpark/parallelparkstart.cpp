...
{   
    ...
    SetPropertyFloat(PP_STG7STR, 116);
    SetPropertyBool(PP_STG7STR NSSUBPROP_ISCHANGEABLE, tTrue);
    SetPropertyStr(PP_STG7STR NSSUBPROP_DESCRIPTION, "stage 7 steering angle");
    ...
    switch (nEventCode)
            ...
            case IPinEventSink::PE_MediaSampleReceived:
				...
				else if (pSource == &m_oDistance)
				{
					...
					//DistVal = Distnce;
					//LOG_INFO(cString::Format("----------READING Demo : distance %f-m------------",Distnce));
...
tResult cParallelParkInit::ProcessVideo(adtf::IMediaSample* pISample)
{
    ...
    if (IS_OK(pISample->Lock(&l_pSrcBuffer)))
    {
			...
			if(!frame_noise)
			{
				...
				{			
					...
					{	
						...
						trigger1 = 1; 
						LOG_INFO(cString::Format("-----aadc trigger 1 main----- %f",distance_begin ));
						total_count++;
						scan = tTrue;
						time_count = 1;
					}
		...
		else
		{
			...
			time_diffd = (float)(end_time - start_time)/1000000;
			if(time_diffd >= step_timed)  // car init delay
			{		
...
//******************************step 1*************************************//
	if(trigger1 == 1)
	{
		...
		//start_time = _clock->GetStreamTime();   //start time
		start_dist = Distnce;  //start distance
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_dist 1 %f-secs------------",start_dist));	
	...
	if(currentStage == 1)
	{
		//end_time = _clock->GetStreamTime();  // current time
		end_dist = Distnce;	//current distance
		accelout = parkaccel;  
		Steerout = Steerin;
		//time_diff = (float)(end_time - start_time)/1000000; // Converted to Secs //in time
		time_diff = (end_dist - start_dist); // Converted to Secs  // in distance
		LOG_INFO(cString::Format("----------Stage1 Demo : distance start %f-m------------",start_dist));
		LOG_INFO(cString::Format("----------Stage1 Demo : time_diff %f-secs------------",time_diff));	 //time
		LOG_INFO(cString::Format("----------Stage1 Demo : distance %f-m------------",end_dist));	//distance
...
//******************************step 2*************************************//
	if(trigger2 == 1)
	{
		...
		start_dist  = Distnce;
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_dist 2 %f-secs------------",start_dist));	
		LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
		trigger2 = 0;  //trigger val
		LOG_INFO(cString::Format("----------Demo : step_time2 %f-secs------------",step_time2));
		distance_scanend = (step_time2*0.3);	// -0.2 instead of *0.8
	...
	if(currentStage == 2)
	{
		...
		time_diff = (end_dist - start_dist);
		LOG_INFO(cString::Format("----------Demo : time_diff %f-secs------------",time_diff));	
		//execution phase
		if((time_diff >= distance_scanend) && (scan)) //scanning
		{
			accelout = 90;
			Steerout = 90;
			// wait for 1 second
			if(time_count == 1)
			{
				start_time = _clock->GetStreamTime();   //start
				time_count = 0;
				LOG_INFO(cString::Format("-----scanning for 1 second-----"));
				time_diffd = 0;
			}
			end_time = _clock->GetStreamTime();
			if(USRight < .45)//sensor distance 
			{
				//spot = tFalse;
				UScount++;
				LOG_INFO(cString::Format("----------Demo : uscount %d -secs------------",UScount));	
			}
			time_diffd = (float)(end_time - start_time)/1000000;
			if(time_diffd  > scan_time)
			{
				if(UScount > 30)
				{
					spot = tFalse;
...
//******************************step 3*************************************//
	if(trigger3 == 1)
	{
		...
		//start_dist = _clock->GetStreamTime();   //start
		start_dist = Distnce;
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_dist 3 %f-secs------------",start_dist));	
		LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
...
//******************************step 4*************************************//
	if(trigger4 == 1)
	{
		...
		start_dist = Distnce;
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_dist 4 %f-secs------------",start_dist));	
		LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
	...
	if(currentStage == 4)
	{
		...
		Steerout = 120;
		//time_diff = (float)(end_dist - start_time)/1000000; // Converted to Secs
		time_diff = (end_dist - start_dist); // Converted to Secs
		LOG_INFO(cString::Format("----------Demo : time_diff %f-secs------------",time_diff));	
...
//******************************step 5*************************************//
	if(trigger5 == 1)
	{
		...
		start_dist = Distnce;
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_dist5 %f-secs------------",start_dist));	
		LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
...
//******************************step 6*************************************//
	if(trigger6 == 1)
	{
		...
		start_dist = Distnce;
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_dist 6 %f-secs------------",start_dist));	
		LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
...
//******************************step 7*************************************//
	if(trigger7 == 1)
	{
		...
		start_dist = Distnce;
		time_diff = 0; 
		LOG_INFO(cString::Format("----------Demo : start_time 7 %f-secs------------",start_dist));	
		LOG_INFO(adtf_util::cString::Format(" %d ", currentStage ));
...
{     
	if(accelout<80 && accelout > 105)
	{	
		accelout = 90;
		Steerout = 90;
		writeOutputs(timeStamp);	
		LOG_INFO(cString::Format("-----Demo : accel error---**STOPPED**-----"));
		RETURN_NOERROR;	
	}  
